...
class Scorelator:
    def __init__(
        ...
        self.intercept_index = (
            self.cov_names.index("intercept")
        ...
        self.draw_lb = self.beta + norm.ppf(
            self.draw_bounds[0], scale=np.sqrt(self.gamma + self.beta_sd**2)
        )
        self.draw_ub = self.beta + norm.ppf(
            self.draw_bounds[1], scale=np.sqrt(self.gamma + self.beta_sd**2)
        )
        self.wider_draw_lb = self.beta + norm.ppf(
            self.draw_bounds[0], scale=np.sqrt(gamma_ub + self.beta_sd**2)
        )
        self.wider_draw_ub = self.beta + norm.ppf(
    ...
    def plot_model(
        ...
        alt_dorms = np.array(
            [dorm for i in range(data.num_obs) for dorm in data.alt_dorms[i]]
        )
        obs_ref = np.array(
            [
                np.sum(
                    [
                        self.model.beta[self.model.var_idx[dorm]]
        ...
        for i, dorm in enumerate(dorms):
            ...
            ax.scatter(
                x, obs[index], s=2.0 / data.obs_se[index], color="gray", alpha=0.5
            )
            ax.fill_between(
                [lb, ub],
                [self.draw_lb[beta_index], self.draw_lb[beta_index]],
                [self.draw_ub[beta_index], self.draw_ub[beta_index]],
                color="#008080",
                alpha=0.2,
            )
            ax.fill_between(
        ...
        ax.set_title(
            f"{title}: ref = {self.model.gold_dorm}\n"
            f"detailed scores: {dict(zip(dorms, list(low_score[beta_sort_id])))}\n"
            f"overall score: {np.max(list(low_score[beta_sort_id]))}",
            loc="left",
        )
