...
class PostAnalysis:
    ...
    def detect_pub_bias(self):
        ...
        self.se_model_all["pval"] = get_p_val(
            self.se_model_all["mean"], self.se_model_all["sd"]
        )
    ...
    def summarize_model(self) -> pd.DataFrame:
        ...
        summary.update(
            {"egger_" + name: value for name, value in self.se_model.items()}
        )
        summary.update(
            {"egger_all_" + name: value for name, value in self.se_model_all.items()}
        )
    ...
    def plot_residual(self, ax: plt.Axes) -> plt.Axes:
        ...
        if fill_index.sum() > 0:
            ax.scatter(
                residual[fill_index], obs_se[fill_index], color="#008080", alpha=0.7
            )
        ax.scatter(
            residual[self.df.outlier == 1],
        ...
        ax.plot(
            [0, -1.96 * max_obs_se], [0.0, max_obs_se], linewidth=1, color="#87CEFA"
        )
        ax.plot(
            [0.0, 1.96 * max_obs_se], [0.0, max_obs_se], linewidth=1, color="#87CEFA"
        )
